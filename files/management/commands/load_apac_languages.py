...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        languages_data = [
            ...
            {
                "code": "automatic",
                "title": "Auto-generated Transcription",
                "legacy_codes": [],
                "legacy_titles": [],
            },
            {
        ...
        for lang_data in languages_data:
            ...
            if options["dry_run"]:
                ...
                if existing:
                    if existing.code != lang_data["code"] and options.get("update"):
                        ...
                        self.stdout.write(
                            f"Would update Language: {lang_data['title']}"
                        )
                    else:
                        self.stdout.write(
                            f"Would skip existing Language: {existing.title}"
                        )
                else:
                ...
                if existing is None:
                    ...
                    Language.objects.create(
                        code=lang_data["code"], title=lang_data["title"]
                    )
                    created_languages += 1
                    self.stdout.write(
                    ...
                    if updated:
                        ...
                        if existing.code == lang_data["code"]:
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Updated Language: {lang_data['title']}"
                                )
                            )
                else:
                    self.stdout.write(
                        self.style.NOTICE(
                            f"Skipped existing Language: {existing.title}"
                        )
                    )
            ...
            if lang_data["code"] not in exclude_from_media_language:
                ...
                lookup_titles = [lang_data["title"]] + lang_data.get(
                    "legacy_titles", []
                )

                if options["dry_run"]:
                    # Check if any of the lookup titles exist
                    existing_media_langs = MediaLanguage.objects.filter(
                        title__in=lookup_titles
                    )
                    if existing_media_langs.exists():
                        for exists in existing_media_langs:
                            if exists.title != lang_data["title"] and options.get(
                                "update"
                            ):
                                self.stdout.write(
                                    f"Would update MediaLanguage: {exists.title} -> {lang_data['title']}"
                                )
                            else:
                                self.stdout.write(
                                    f"Would skip existing MediaLanguage: {exists.title}"
                                )
                    else:
                        self.stdout.write(
                            f"Would create MediaLanguage: {lang_data['title']}"
                        )
                else:
                    ...
                    if not existing_media_langs.exists():
                        ...
                        if target_exists:
                            ...
                            target = (
                                MediaLanguage.objects.filter(title=lang_data["title"])
                                .order_by("id")
                                .first()
                            )
                            ...
                            if target is None:
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"Could not find MediaLanguage with title: {lang_data['title']}"
                                    )
                                )
                                continue

                            duplicates_to_merge = existing_media_langs.exclude(
                                id=target.id
                            )
                            ...
                            if duplicates_to_merge.exists() and options["update"]:
                                ...
                                self.stdout.write(
                                    self.style.SUCCESS(
                                        f"Merged duplicates into existing MediaLanguage: {target.title}"
                                    )
                                )
                            else:
                                self.stdout.write(
                                    self.style.NOTICE(
                                        f"Skipped existing MediaLanguage: {target.title}"
                                    )
                                )
                        else:
                            ...
                            if (
                                ...
                                self.stdout.write(
                                    self.style.NOTICE(
                                        f"Skipped existing MediaLanguage: {primary.title}"
                                    )
                                )
            ...
            for old_code, new_code in consolidation_map.items():
                try:
                    ...
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"Target language {new_code} does not exist, skipping {old_code}"
                            )
                        )
                        continue

                    # Count affected subtitles
                    subtitle_count = Subtitle.objects.filter(
                    ...
                    if options["dry_run"]:
                        ...
                        self.stdout.write(
                            f"Would remove language: {old_code} ({old_language.title})"
                        )
                    else:
                        # Migrate subtitles
                        if subtitle_count > 0:
                            updated = Subtitle.objects.filter(
                                language=old_language
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"Removed duplicate language: {old_code} ({old_title})"
                            )
                        )
