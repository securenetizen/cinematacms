...
export default class MediaItemPreviewer {
	...
	constructor(extensions) {
		...
		if (this.extensions.anim.length || this.extensions.fallback) {
		...
		if (this.extensions.anim.length) {
			...
			while (i < this.extensions.anim.length) {
				this.extensions.anim[i].
					elem.setAttribute('srcset', src_parts[0] + '.' + this.extensions.anim[i].type + (src_parts[1] ? '?' + src_parts[1] : ''));
				i += 1;
			}
		}
		if (this.extensions.fallback) {
			this.extensions.fallback.elem.setAttribute('src', src_parts[0] + '.' + this.extensions.fallback.type + (src_parts[1] ? '?' + src_parts[1] : ''));
		}
